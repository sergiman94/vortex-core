...
import com.vortex.common.iterator.LimitIterator;
...
public class VortexTraverser {

    protected static final Logger LOG = Log.logger(VortexTraverser.class);
    ...
    public VortexTraverser(Vortex graph) {
        this.graph = graph;
    ...
    protected Set<Id> adjacentVertices(Id sourceV, Set<Id> vertices,
        ...
        for (Id source : vertices) {
            Iterator<Edge> edges = this.edgesOfVertex(source, dir,
                                                      label, degree);
            while (edges.hasNext()) {
                VortexEdge e = (VortexEdge) edges.next();
    ...
    protected Set<Id> adjacentVertices(Id source, EdgeStep step) {
        ...
        while (edges.hasNext()) {
            neighbors.add(((VortexEdge) edges.next()).id().otherVertexId());
        }
        return neighbors;
    }
    ...
    private Iterator<Edge> edgesOfVertex(Id source, EdgeStep edgeStep,
        ...
        if (filter != null) {
            ...
            edges = new FilterIterator<>(edges, (e) -> {
                return finalFilter.test((VortexEdge) e);
            });
        }
        return edgeStep.skipSuperNodeIfNeeded(edges);
    }
    ...
    private void fillFilterBySortKeys(Query query, Id[] edgeLabels,
        ...
        if (!GraphTransaction.matchFullEdgeSortKeys(condQuery, this.graph())) {
            Id label = condQuery.condition(VortexKeys.LABEL);
    ...
    protected Object getVertexLabelId(Object label) {
        ...
        return SchemaLabel.getLabelId(this.graph, VortexType.VERTEX, label);
    }
    ...
    protected Id getEdgeLabelId(Object label) {
        ...
        return SchemaLabel.getLabelId(this.graph, VortexType.EDGE, label);
    }
    ...
    public static void checkCapacity(long capacity, long access,
        ...
        if (capacity != NO_LIMIT && access > capacity) {
            throw new VortexException("Exceed capacity '%s' while finding %s",
                                    capacity, traverse);
        }
    }
    ...
    public static class Node {
        ...
        public List<Id> joinPath(Node back) {
            return VortexTraverser.joinPath(this, back, false);
        }
