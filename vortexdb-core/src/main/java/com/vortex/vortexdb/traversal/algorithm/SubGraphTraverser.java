...
public class SubGraphTraverser extends VortexTraverser {

    public SubGraphTraverser(Vortex graph) {
        super(graph);
    }
    ...
    private static boolean hasMultiEdges(List<Edge> edges, Id target) {
        ...
        for (Edge edge : edges) {
            if (((VortexEdge) edge).id().otherVertexId().equals(target)) {
                if (((VortexEdge) edge).direction() == Directions.OUT) {
                    hasOutEdge = true;
                } else {
                    hasInEdge = true;
                }
