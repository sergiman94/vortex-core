...
public final class TraversalUtil {
    ...
    public static Vortex getGraph(Step<?, ?> step) {
        return (Vortex) step.getTraversal().getGraph().get();
    }

    public static void extractHasContainer(VortexStep<?, ?> newStep,
    ...
    public static void extractHasContainer(VortexVertexStep<?> newStep,
                                           Traversal.Admin<?, ?> traversal) {
        Step<?, ?> step = newStep;
        do {
            if (step instanceof HasStep) {
    ...
    public static ConditionQuery fillConditionQuery(
                                 ...
                                 Vortex graph) {
        VortexType resultType = query.resultType();
    ...
    public static Condition convAnd(Vortex graph,
                                    VortexType type,
                                    HasContainer has) {
        P<?> p = has.getPredicate();
    ...
    public static Condition convOr(Vortex graph,
                                   VortexType type,
                                   HasContainer has) {
        P<?> p = has.getPredicate();
    ...
    private static Relation convCompare2Relation(Vortex graph,
                                                 VortexType type,
                                                 HasContainer has) {
        assert type.isGraph();
    ...
    private static Relation convCompare2SyspropRelation(Vortex graph,
                                                        VortexType type,
                                                        HasContainer has) {
        BiPredicate<?, ?> bp = has.getPredicate().getBiPredicate();
        assert bp instanceof Compare;

        VortexKeys key = token2vortexKey(has.getKey());
    ...
    private static Relation convCompare2UserpropRelation(Vortex graph,
                                                         VortexType type,
                                                         HasContainer has) {
        BiPredicate<?, ?> bp = has.getPredicate().getBiPredicate();
        assert bp instanceof Compare;
    ...
    private static Condition convRelationType2Relation(Vortex graph,
                                                       VortexType type,
                                                       HasContainer has) {
        assert type.isGraph();
        BiPredicate<?, ?> bp = has.getPredicate().getBiPredicate();
    ...
    public static Condition convIn2Relation(Vortex graph,
                                            VortexType type,
                                            HasContainer has) {
        BiPredicate<?, ?> bp = has.getPredicate().getBiPredicate();
        ...
        VortexKeys vortexKey = token2vortexKey(originKey);
        List<?> valueList;
        if (vortexKey != null) {
            valueList = convSysListValueIfNeeded(graph, type, vortexKey, values);
            switch ((Contains) bp) {
                case within:
                    return Condition.in(vortexKey, valueList);
                case without:
                    return Condition.nin(vortexKey, valueList);
            }
        } else {
    ...
    public static Condition convContains2Relation(Vortex graph,
                                                  HasContainer has) {
    ...
    public static VortexKeys string2vortexKey(String key) {
        VortexKeys vortexKey = token2vortexKey(key);
        return vortexKey != null ? vortexKey : VortexKeys.valueOf(key);
    }

    public static VortexKeys token2vortexKey(String key) {
        if (key.equals(T.label.getAccessor())) {
            return VortexKeys.LABEL;
        } else if (key.equals(T.id.getAccessor())) {
            return VortexKeys.ID;
        } else if (keyForContainsKeyOrValue(key)) {
            return VortexKeys.PROPERTIES;
        }
        return null;
    }
    ...
    public static void convAllHasSteps(Traversal.Admin<?, ?> traversal) {
        ...
        Vortex graph = (Vortex) traversal.getGraph().get();
        for (HasStep<?> step : steps) {
            TraversalUtil.convHasStep(graph, step);
        }
    }

    public static void convHasStep(Vortex graph, HasStep<?> step) {
    ...
    private static void convPredicateValue(Vortex graph,
                                           HasContainer has) {
    ...
    private static boolean isSysProp(String key) {
        ...
        return token2vortexKey(key) != null;
    }
    ...
    private static Object convSysValueIfNeeded(Vortex graph,
                                               VortexType type,
                                               VortexKeys key,
                                               Object value) {
        if (key == VortexKeys.LABEL && !(value instanceof Id)) {
            value = SchemaLabel.getLabelId(graph, type, value);
        } else if (key == VortexKeys.ID && !(value instanceof Id)) {
            value = VortexElement.getIdValue(type, value);
        }
        return value;
    }

    private static List<?> convSysListValueIfNeeded(Vortex graph,
                                                    VortexType type,
                                                    VortexKeys key,
                                                    Collection<?> values) {
    ...
    public static boolean testProperty(Property<?> prop, Object expected) {
        ...
        updatePredicateValue(predicate, ((VortexProperty<?>) prop).propertyKey());
        return predicate.test(actual);
    }

    public static Map<Id, Object> transProperties(Vortex graph,
    ...
    public static P<Object> parsePredicate(String predicate) {
        ...
        if (!matcher.find()) {
            throw new VortexException("Invalid predicate: %s", predicate);
        }
    ...
    private static Number[] predicateNumbers(String value, int count) {
        ...
        if (values.size() != count) {
            throw new VortexException("Invalid numbers size %s, expect %s",
                                    values.size(), count);
        }
