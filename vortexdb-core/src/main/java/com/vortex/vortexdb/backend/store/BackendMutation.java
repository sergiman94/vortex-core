...
public class BackendMutation {
    ...
    private static VortexException incompatibleActionException(
            Action newAction,
            Action originAction) {
        return new VortexException("The action '%s' is incompatible with " +
     ...
     * @return mutations
     */
    public Iterator<BackendAction> mutation(VortexType type) {
     ...
     * @return mutations
     */
    public List<BackendAction> mutation(VortexType type, Id id) {
     ...
     * @return true if exist, otherwise false
     */
    public boolean contains(VortexType type, Action action) {
    ...
    private static class MutationTable {
        ...
        private final Map<VortexType, Map<Id, List<BackendAction>>> mutations;
        ...
        public void put(VortexType type, Id id, BackendAction mutation) {
        ...
        public boolean containsKey(VortexType type, Id id) {
            Map<Id, List<BackendAction>> table = this.mutations.get(type);
            return table != null && table.containsKey(id);
        }

        public List<BackendAction> get(VortexType type, Id id) {
            Map<Id, List<BackendAction>> table = this.mutations.get(type);
            if (table == null) {
                return null;
            }
        ...
        public Iterator<BackendAction> get(VortexType type) {
