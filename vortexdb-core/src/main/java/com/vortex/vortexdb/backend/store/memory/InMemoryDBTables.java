...
public class InMemoryDBTables {
    ...
    public static class Vertex extends InMemoryDBTable {
        ...
        public Vertex() {
            super(VortexType.VERTEX);
        }
    }

    public static class Edge extends InMemoryDBTable {

        public Edge(VortexType type) {
            super(type);
        }
        ...
        public void insert(BackendSession session, TextBackendEntry entry) {
            ...
            if (!this.store().containsKey(id)) {
                BackendEntry vertex = new TextBackendEntry(VortexType.VERTEX, id);
                vertex.merge(entry);
        ...
        protected Map<Id, BackendEntry> queryByIdPrefix(
            ...
            BackendEntry edges = new TextBackendEntry(VortexType.VERTEX,
                                                      entry.id());
        ...
        protected Map<Id, BackendEntry> queryByFilter(
            ...
            E.checkState(relation.key().equals(VortexKeys.LABEL),
                         "Not support querying edge by %s", conditions);
            ...
            for (BackendEntry value : entries.values()) {
                // TODO: Compatible with BackendEntry
                TextBackendEntry entry = (TextBackendEntry) value;
                String out = EdgeId.concat(VortexType.EDGE_OUT.string(), label);
                String in = EdgeId.concat(VortexType.EDGE_IN.string(), label);
    ...
    public static class SecondaryIndex extends InMemoryDBTable {
        ...
        public SecondaryIndex() {
            super(VortexType.SECONDARY_INDEX);
        }

        protected SecondaryIndex(VortexType type) {
            super(type);
        }
        ...
        public Iterator<BackendEntry> query(BackendSession session,
            ...
            for (Condition c : conditions) {
                ...
                if (r.key() == VortexKeys.FIELD_VALUES) {
                    fieldValue = r.value().toString();
                } else if (r.key() == VortexKeys.INDEX_LABEL_ID) {
            ...
            Id id = VortexIndex.formatIndexId(query.resultType(),
        ...
        public void delete(BackendSession session, TextBackendEntry entry) {
            ...
            String indexLabel = entry.column(VortexKeys.INDEX_LABEL_ID);
    ...
    public static class UniqueIndex extends SecondaryIndex {
        ...
        public UniqueIndex() {
            super(VortexType.UNIQUE_INDEX);
        }
    }

    public static class RangeIndex extends InMemoryDBTable {

        protected RangeIndex(VortexType type) {
        ...
        public Iterator<BackendEntry> query(BackendSession session,
            ...
            for (Condition.Relation r : ((ConditionQuery) query).relations()) {
                if (r.key().equals(VortexKeys.INDEX_LABEL_ID)) {
                    indexLabelId = (Id) r.value();
                    continue;
                }
                relations.add(r);
        ...
        private Iterator<BackendEntry> betweenQuery(Id indexLabelId,
            ...
            Id min = VortexIndex.formatIndexId(type, indexLabelId, keyMin);
            ...
            Id max = VortexIndex.formatIndexId(type, indexLabelId, keyMax);
        ...
        public void delete(BackendSession session, TextBackendEntry entry) {
            ...
            Id min = VortexIndex.formatIndexId(entry.type(), indexLabelId, 0L);
            indexLabelId = IdGenerator.of(indexLabelId.asLong() + 1L);
            Id max = VortexIndex.formatIndexId(entry.type(), indexLabelId, 0L);
        ...
        public static RangeIndex rangeInt() {
            return new RangeIndex(VortexType.RANGE_INT_INDEX);
        }

        public static RangeIndex rangeFloat() {
            return new RangeIndex(VortexType.RANGE_FLOAT_INDEX);
        }

        public static RangeIndex rangeLong() {
            return new RangeIndex(VortexType.RANGE_LONG_INDEX);
        }

        public static RangeIndex rangeDouble() {
            return new RangeIndex(VortexType.RANGE_DOUBLE_INDEX);
        }
    }
