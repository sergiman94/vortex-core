...
import com.vortex.vortexdb.backend.page.*;
import com.vortex.vortexdb.backend.page.IdHolder.BatchIdHolder;
import com.vortex.vortexdb.backend.page.IdHolder.FixedIdHolder;
import com.vortex.vortexdb.backend.page.IdHolder.PagingIdHolder;
...
import com.vortex.vortexdb.exception.NoIndexException;
...
import com.vortex.vortexdb.util.*;
...
public class GraphIndexTransaction extends AbstractTransaction {
    ...
    public GraphIndexTransaction(VortexParams graph, BackendStore store) {
        super(graph, store);
    ...
    public void updateLabelIndex(VortexElement element, boolean removed) {
        if (element instanceof VortexVertex && ((VortexVertex) element).olap()) {
            return;
        }
        ...
        VortexIndex index = new VortexIndex(this.graph(),
    ...
    @Watched(prefix = "index")
    public void updateVertexIndex(VortexVertex vertex, boolean removed) {
        if (vertex.olap()) {
    ...
    @Watched(prefix = "index")
    public void updateEdgeIndex(VortexEdge edge, boolean removed) {
    ...
    private void updateVertexOlapIndex(VortexVertex vertex, boolean removed) {
     ...
     */
    protected void updateIndex(Id ilId, VortexElement element, boolean removed) {
        ...
        for (Id fieldId : indexLabel.indexFields()) {
            VortexProperty<Object> property = element.getProperty(fieldId);
            if (property == null) {
    ...
    private boolean existUniqueValueInStore(IndexLabel indexLabel,
                                            Object value) {
        ConditionQuery query = new ConditionQuery(VortexType.UNIQUE_INDEX);
        query.eq(VortexKeys.INDEX_LABEL_ID, indexLabel.id());
        query.eq(VortexKeys.FIELD_VALUES, value);
        boolean exist;
        Iterator<BackendEntry> iterator = this.query(query).iterator();
        try {
            exist = iterator.hasNext();
            if (exist) {
                VortexIndex index = this.serializer.readIndex(graph(), query,
                                                            iterator.next());
    ...
    public IdHolderList queryIndex(ConditionQuery query) {
        ...
        if (this.hasUpdate()) {
            throw new VortexException("Can't do index query when " +
        ...
        if (conds.size() > 1 ||
            (conds.size() == 1 && !query.containsCondition(VortexKeys.LABEL))) {
            throw new VortexException("Can't do index query with %s and %s",
        ...
        if (query.allSysprop() && conds.size() == 1 &&
            query.containsCondition(VortexKeys.LABEL)) {
            // Query only by label
    ...
    private IdHolderList queryByLabel(ConditionQuery query) {
        ...
        indexQuery.eq(VortexKeys.FIELD_VALUES, label);
        /*
    ...
    private void recordIndexValue(ConditionQuery query, VortexIndex index) {
        ...
        query.registerResultsFilter(elem -> {
            for (Condition cond : originQuery.conditions()) {
                ...
                if (key instanceof Id && indexFields.contains(key)) {
                    ...
                    Id field = (Id) key;
                    assert elem != null;
                    VortexProperty<?> property = elem.getProperty(field);
    ...
    private void removeExpiredIndexIfNeeded(VortexIndex index,
                                            boolean showExpired) {
        if (this.store().features().supportsTtl() || showExpired) {
            return;
        }
        for (VortexIndex.IdWithExpiredTime id : index.expiredElementIds()) {
            VortexIndex removeIndex = index.clone();
    ...
    private static ConditionQuery constructQuery(ConditionQuery query,
        ...
        switch (indexType) {
            case SEARCH:
                ...
                indexQuery.eq(VortexKeys.INDEX_LABEL_ID, indexLabel.id());
                indexQuery.eq(VortexKeys.FIELD_VALUES, fieldValue);
                break;
            case SECONDARY:
                ...
                indexQuery = new ConditionQuery(indexType.type(), query);
                indexQuery.eq(VortexKeys.INDEX_LABEL_ID, indexLabel.id());
                indexQuery.eq(VortexKeys.FIELD_VALUES, joinedValues);
                break;
            case RANGE_INT:
            case RANGE_FLOAT:
            case RANGE_LONG:
            case RANGE_DOUBLE:
                ...
                break;
            case SHARD:
                VortexType type = indexLabel.indexType().type();
    ...
    protected static List<Condition> constructShardConditions(
         ...
         * layer can do filter.
         */
        if (key == VortexKeys.FIELD_VALUES &&
            processedCondCount < query.userpropKeys().size()) {
            throw new VortexException("Invalid shard index query: %s", query);
        }
    ...
    private static Relation shardFieldValuesCondition(VortexKeys key,
    ...
    private static boolean validQueryConditionValues(Vortex graph,
                                                     ConditionQuery query) {
    ...
    private static NoIndexException noIndexException(Vortex graph,
                                                     ConditionQuery query,
                                                     Id label) {
    ...
    private static boolean hasNullableProp(VortexElement element, Id key) {
        return element.schemaLabel().nullableKeys().contains(key);
    }

    private static Set<IndexLabel> relatedIndexLabels(VortexElement element) {
        Set<IndexLabel> indexLabels = InsertionOrderUtil.newSet();
    ...
    private static class IndexQueries
        ...
        private static class JointQuery extends Query {
            ...
            public JointQuery(VortexType type, Collection<Query> queries) {
    ...
    public static class RemoveLeftIndexJob extends EphemeralJob<Object> {
        ...
        private final VortexElement element;
        private GraphIndexTransaction tx;
        private Set<ConditionQuery.LeftIndex> leftIndexes;

        private RemoveLeftIndexJob(ConditionQuery query, VortexElement element) {
        ...
        protected long removeIndexLeft(ConditionQuery query,
                                       VortexElement element) {
            if (element.type() != VortexType.VERTEX &&
                element.type() != VortexType.EDGE_OUT &&
                element.type() != VortexType.EDGE_IN) {
                throw new VortexException("Only accept element of type VERTEX " +
        ...
        private long processSecondaryOrSearchIndexLeft(ConditionQuery query,
            ...
            VortexElement deletion = this.constructErrorElem(query, element,
                                                           incorrectPKs);
            if (deletion == null) {
                return 0;
            }
        ...
        private VortexElement constructErrorElem(
                            ConditionQuery query, VortexElement element,
                            Map<PropertyKey, Object> incorrectPKs) {
            VortexElement errorElem = element.copyAsFresh();
            ...
            for (Id key : propKeys) {
                ...
                VortexProperty<?> prop = element.getProperty(key);
        ...
        private boolean deletedByError(ConditionQuery query,
                                       VortexElement element) {
            VortexElement elem = this.newestElement(element);
            if (elem == null) {
                return false;
            }
        ...
        private boolean deletedByError(VortexElement element,
        ...
        private VortexElement newestElement(VortexElement element) {
            boolean isVertex = element instanceof VortexVertex;
            if (isVertex) {
                Iterator<Vertex> iter = this.graph().vertices(element.id());
                return (VortexVertex) QueryResults.one(iter);
            } else {
                assert element instanceof VortexEdge;
                Iterator<Edge> iter = this.graph().edges(element.id());
                return (VortexEdge) QueryResults.one(iter);
            }
        }
    }
}
