...
import com.vortex.vortexdb.backend.query.Condition.SyspropRelation;
...
public final class ConditionQueryFlatten {
    ...
    private static final Set<VortexKeys> SPECIAL_KEYS = ImmutableSet.of(
            VortexKeys.LABEL
    );
    ...
    private static Condition convIn2Or(Relation relation, boolean supportIn) {
        ...
        if (supportIn && relation.isSysprop() && values.size() > 1 &&
            (key == VortexKeys.OWNER_VERTEX || key == VortexKeys.ID)) {
            // TODO: Should not rely on VortexKeys here, improve key judgment
            // Just mark flatten
        ...
        for (Object value : values) {
            Condition cond;
            // Convert IN to EQ
            if (key instanceof VortexKeys) {
                cond = Condition.eq((VortexKeys) key, value);
            } else {
    ...
    private static Condition convNotin2And(Relation relation) {
        ...
        for (Object value : values) {
            if (key instanceof VortexKeys) {
                cond = Condition.neq((VortexKeys) key, value);
            } else {
