...
import com.vortex.vortexdb.backend.query.ConditionQueryFlatten;
...
import com.vortex.vortexdb.util.Consumers;
...
public final class RamTable {
    ...
    public RamTable(Vortex graph) {
        this(graph, VERTICES_CAPACITY, EDGES_CAPACITY);
    }

    public RamTable(Vortex graph, long maxVertices, int maxEdges) {
        this.graph = graph;
    ...
    public void reload(boolean loadFromFile, String file) {
        if (this.loading) {
            throw new VortexException("There is one loading task, " +
        ...
        try {
            ...
            this.reset();
            throw new VortexException("Failed to load ramtable", e);
        } finally {
            this.loading = false;
        }
    }
    ...
    private void loadFromDB() throws Exception {
        Query query = new Query(VortexType.VERTEX);
        ...
        while (vertices.hasNext()) {
            ...
            if (vertex.compareTo(lastId) < 0) {
                throw new VortexException("The ramtable feature is not " +
            ...
            if (adjEdges.hasNext()) {
                VortexEdge edge = (VortexEdge) adjEdges.next();
    ...
    public void addEdge(boolean newVertex, VortexEdge edge) {
        if (edge.schemaLabel().existSortKeys()) {
            throw new VortexException("Only edge label without sortkey is " +
    ...
    public boolean matched(Query query) {
        ...
        Object owner = cq.condition(VortexKeys.OWNER_VERTEX);
    ...
    @Watched
    public Iterator<VortexEdge> query(Query query) {
    ...
    private Iterator<VortexEdge> query(ConditionQuery query) {
        Id owner = query.condition(VortexKeys.OWNER_VERTEX);
        assert owner != null;
        Directions dir = query.condition(VortexKeys.DIRECTION);
        if (dir == null) {
            dir = Directions.BOTH;
        }
    ...
    @Watched
    public Iterator<VortexEdge> query(long owner, Directions dir, int label) {
        if (this.loading) {
    ...
    private void vertexAdjPosition(long vertex, int position) {
        if (vertex < this.verticesCapacityHalf) {
            ...
            throw new VortexException("Out of vertices capaticy %s",
    ...
    private int vertexAdjPosition(long vertex) {
        if (vertex < this.verticesCapacityHalf) {
            ...
            throw new VortexException("Out of vertices capaticy %s: %s",
    ...
    private static void ensureNumberId(Id id) {
        if (!id.number()) {
            throw new VortexException("Only number id is supported by " +
    ...
    private class EdgeRangeIterator implements Iterator<VortexEdge> {

        private final int end;
        private final Directions dir;
        private final int label;
        private final VortexVertex owner;
        private int current;
        private VortexEdge currentEdge;
        ...
        public EdgeRangeIterator(int start, int end,
            ...
            this.dir = dir;
            this.label = label;
            this.owner = new VortexVertex(RamTable.this.graph,
        ...
        @Override
        public VortexEdge next() {
            if (!this.hasNext()) {
                throw new NoSuchElementException();
            }
            assert this.currentEdge != null;
            VortexEdge edge = this.currentEdge;
            this.currentEdge = null;
            return edge;
        }

        private VortexEdge fetch() {
            ...
            Vortex graph = RamTable.this.graph;
            ...
            VortexEdge edge = VortexEdge.constructEdge(this.owner, direction,
    ...
    private class LoadTraverser implements AutoCloseable {
        ...
        private void addVertex(Id vertex) {
            ...
            if (!vertex.number()) {
                throw new VortexException("Only number id is supported " +
        ...
        private void addEdgesByBatch() {
            ...
            for (Id vertex : this.vertices) {
                ...
                for (int i = 0; i < adjEdges.size(); i++) {
                    VortexEdge edge = (VortexEdge) adjEdges.get(i);
