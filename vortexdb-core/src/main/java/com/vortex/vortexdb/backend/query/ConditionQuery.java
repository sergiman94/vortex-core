...
public class ConditionQuery extends IdQuery {
    ...
    private Function<VortexElement, Boolean> resultsFilter = null;
    private Element2IndexValueMap element2IndexValueMap = null;

    public ConditionQuery(VortexType resultType) {
        super(resultType);
    }

    public ConditionQuery(VortexType resultType, Query originQuery) {
        super(resultType, originQuery);
    }

    public ConditionQuery query(Condition condition) {
        // Query by id (Vortex-259)
        if (condition instanceof Relation) {
            Relation relation = (Relation) condition;
            if (relation.key().equals(VortexKeys.ID) &&
    ...
    public ConditionQuery eq(VortexKeys key, Object value) {
        // Filter value by key
        return this.query(Condition.eq(key, value));
    }

    public ConditionQuery gt(VortexKeys key, Object value) {
        return this.query(Condition.gt(key, value));
    }

    public ConditionQuery gte(VortexKeys key, Object value) {
        return this.query(Condition.gte(key, value));
    }

    public ConditionQuery lt(VortexKeys key, Object value) {
        return this.query(Condition.lt(key, value));
    }

    public ConditionQuery lte(VortexKeys key, Object value) {
        return this.query(Condition.lte(key, value));
    }

    public ConditionQuery neq(VortexKeys key, Object value) {
        return this.query(Condition.neq(key, value));
    }

    public ConditionQuery prefix(VortexKeys key, Id value) {
        return this.query(Condition.prefix(key, value));
    }

    public ConditionQuery key(VortexKeys key, Object value) {
    ...
    public boolean containsCondition(VortexKeys key) {
        for (Condition c : this.conditions) {
            if (c.isRelation()) {
                Condition.Relation r = (Condition.Relation) c;
                if (r.key().equals(key)) {
    ...
    public boolean containsRelation(VortexKeys key, Condition.RelationType type) {
        for (Relation r : this.relations()) {
    ...
    public List<Condition> syspropConditions(VortexKeys key) {
        this.checkFlattened();
    ...
    public boolean mayHasDupKeys(Set<VortexKeys> keys) {
        Map<VortexKeys, Integer> keyCounts = new HashMap<>();
        for (Condition condition : this.conditions) {
            ...
            if (keys.contains(relation.key())) {
                ...
                keyCounts.put((VortexKeys) relation.key(), keyCount);
            }
        }
        return false;
    }
    ...
    public void registerResultsFilter(Function<VortexElement, Boolean> filter) {
    ...
    public static final class Element2IndexValueMap {
        ...
        public boolean validRangeIndex(VortexElement element, Condition cond) {
            ...
            VortexProperty<Object> vortexProperty = element.getProperty(propId);
            if (vortexProperty == null) {
             ...
             */
            boolean hasRightValue = removeValue(fieldValues, vortexProperty.value());
