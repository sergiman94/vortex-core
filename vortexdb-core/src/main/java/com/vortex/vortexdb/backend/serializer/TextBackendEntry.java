...
public class TextBackendEntry implements BackendEntry, Cloneable {
    ...
    private final VortexType type;
    private final Id id;
    private Id subId;
    private NavigableMap<String, String> columns;

    public TextBackendEntry(VortexType type, Id id) {
        this.type = type;
        this.id = id;
        this.subId = null;
        this.resetColumns();
    ...
    public void column(VortexKeys column, String value) {
    ...
    public String column(VortexKeys column) {
    ...
    public void append(TextBackendEntry entry) {
        for (Entry<String, String> col : entry.columns.entrySet()) {
            ...
            if (col.getKey().startsWith(VortexType.PROPERTY.string())) {
                this.columns.put(col.getKey(), col.getValue());
                continue;
            }

            // TODO: use more general method
            if (!col.getKey().endsWith(VortexKeys.ELEMENT_IDS.string())) {
                continue;
            }
    ...
    public void eliminate(TextBackendEntry entry) {
        for (Entry<String, String> col : entry.columns.entrySet()) {
            ...
            // TODO: use more general method
            if (col.getKey().startsWith(VortexType.PROPERTY.string()) ||
                col.getKey().startsWith(VortexType.EDGE_OUT.string()) ||
                col.getKey().startsWith(VortexType.EDGE_IN.string())) {
