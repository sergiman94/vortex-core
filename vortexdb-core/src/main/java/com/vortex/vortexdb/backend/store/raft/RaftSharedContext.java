...
import com.vortex.vortexdb.backend.store.raft.rpc.ListPeersProcessor;
...
import com.vortex.vortexdb.backend.store.raft.rpc.SetLeaderProcessor;
import com.vortex.vortexdb.backend.store.raft.rpc.StoreCommandProcessor;
...
public final class RaftSharedContext {
    ...
    public RaftSharedContext(VortexParams params) {
        this.params = params;
    ...
    public NodeOptions nodeOptions() throws IOException {
        ...
        if (!groupPeers.parse(groupPeersStr)) {
            throw new VortexException("Failed to parse group peers %s",
                                    groupPeersStr);
        }
    ...
    protected void clearCache() {
        ...
        this.notifyCache(Cache.ACTION_CLEAR, VortexType.VERTEX_LABEL, null);
        this.notifyCache(Cache.ACTION_CLEAR, VortexType.VERTEX, null);
    }
    ...
    protected void updateCacheIfNeeded(BackendMutation mutation,
        ...
        for (VortexType type : mutation.types()) {
    ...
    protected void notifyCache(String action, VortexType type, List<Id> ids) {
        EventHub eventHub;
        if (type.isGraph()) {
    ...
    public PeerId endpoint() {
        ...
        if (!endpoint.parse(endpointStr)) {
            throw new VortexException("Failed to parse endpoint %s", endpointStr);
        }
        return endpoint;
    }
