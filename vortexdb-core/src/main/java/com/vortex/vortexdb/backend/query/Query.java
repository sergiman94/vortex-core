...
import com.vortex.vortexdb.util.collection.IdSet;
...
public class Query implements Cloneable {
    ...
    public static final long DEFAULT_CAPACITY = 800000L; // Vortex-777

    private static final ThreadLocal<Long> capacityContext = new ThreadLocal<>();

    protected static final Query NONE = new Query(VortexType.UNKNOWN);

    private static final Set<Id> EMPTY_OLAP_PKS = ImmutableSet.of();

    private VortexType resultType;
    private Map<VortexKeys, Order> orders;
    private long offset;
    ...
    public Query(VortexType resultType) {
        this(resultType, null);
    }

    public Query(VortexType resultType, Query originQuery) {
    ...
    public VortexType resultType() {
        return this.resultType;
    }

    public void resultType(VortexType resultType) {
        this.resultType = resultType;
    }
    ...
    public Map<VortexKeys, Order> orders() {
        return Collections.unmodifiableMap(this.getOrNewOrders());
    }

    public void orders(Map<VortexKeys, Order> orders) {
        this.orders = InsertionOrderUtil.newMap(orders);
    }

    public void order(VortexKeys key, Order order) {
        this.getOrNewOrders().put(key, order);
    }

    protected Map<VortexKeys, Order> getOrNewOrders() {
        if (this.orders != null) {
            return this.orders;
        }
