...
import com.vortex.common.util.TimeUtil;
import org.slf4j.Logger;

import java.util.Map;
import java.util.concurrent.ConcurrentHashMap;

// TODO: learn about snowflake
public class SnowflakeIdGenerator extends IdGenerator {
    ...
    public static SnowflakeIdGenerator init(VortexParams graph) {
    ...
    public static SnowflakeIdGenerator instance(Vortex graph) {
        String graphName = graph.name();
        SnowflakeIdGenerator generator = INSTANCES.get(graphName);
    ...
    private SnowflakeIdGenerator(VortexConfig config) {
    ...
    public Id generate() {
        if (this.idWorker == null) {
            throw new VortexException("Please initialize before using");
        }
    ...
    @Override
    public Id generate(VortexVertex vertex) {
        ...
        public synchronized long nextId() {
            ...
            if (timestamp > this.lastTimestamp) {
                ...
                LOG.error("Clock is moving backwards, " +
                          ...
                          this.lastTimestamp);
                throw new VortexException("Clock moved backwards. Refusing to " +
