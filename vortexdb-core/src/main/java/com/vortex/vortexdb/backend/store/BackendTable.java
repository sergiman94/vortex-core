...
public abstract class BackendTable<Session extends BackendSession, Entry> {
    ...
    private final com.vortex.vortexdb.backend.store.MetaDispatcher<Session> dispatcher;

    public BackendTable(String table) {
        this.table = table.toLowerCase();
        this.dispatcher = new com.vortex.vortexdb.backend.store.MetaDispatcher<>();
    ...
    public com.vortex.vortexdb.backend.store.MetaDispatcher<Session> metaDispatcher() {
     ...
     */
    public static VortexType tableType(Query query) {
        VortexType type = query.resultType();

        // Mapping EDGE to EDGE_OUT/EDGE_IN
        if (type == VortexType.EDGE) {
            // We assume query OUT edges
            type = VortexType.EDGE_OUT;
            ...
            if (query.conditionsSize() > 0 && query instanceof ConditionQuery) {
                ...
                // Does query IN edges
                if (cq.condition(VortexKeys.DIRECTION) == Directions.IN) {
                    type = VortexType.EDGE_IN;
                }
            }
        }
