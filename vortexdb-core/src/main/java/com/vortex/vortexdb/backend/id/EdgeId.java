...
public class EdgeId implements Id {
    ...
    public static final VortexKeys[] KEYS = new VortexKeys[] {
            VortexKeys.OWNER_VERTEX,
            VortexKeys.DIRECTION,
            VortexKeys.LABEL,
            VortexKeys.SORT_VALUES,
            VortexKeys.OTHER_VERTEX
    };
    ...
    public EdgeId(VortexVertex ownerVertex, Directions direction,
                  Id edgeLabelId, String sortValues, VortexVertex otherVertex) {
    ...
    public static Directions directionFromCode(byte code) {
        return Directions.convert(VortexType.fromCode(code));
    }

    public static boolean isOutDirectionFromCode(byte code) {
        return code == VortexType.EDGE_OUT.code();
    }
    ...
    public static EdgeId parse(String id, boolean returnNullIfError)
        ...
        try {
            if (idParts.length == 4) {
                ...
                Id ownerVertexId = IdUtil.readString(idParts[0]);
                VortexType direction = VortexType.fromString(idParts[1]);
