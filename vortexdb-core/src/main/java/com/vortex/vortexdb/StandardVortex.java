...
import com.vortex.vortexdb.backend.cache.CacheNotifier;
import com.vortex.vortexdb.backend.cache.CacheNotifier.GraphCacheNotifier;
import com.vortex.vortexdb.backend.cache.CacheNotifier.SchemaCacheNotifier;
import com.vortex.vortexdb.backend.cache.CachedGraphTransaction;
import com.vortex.vortexdb.backend.cache.CachedSchemaTransaction;
import com.vortex.vortexdb.backend.id.Id;
import com.vortex.vortexdb.backend.id.IdGenerator;
import com.vortex.vortexdb.backend.id.SnowflakeIdGenerator;
...
import com.vortex.vortexdb.io.VortexIoRegistry;
...
import com.vortex.vortexdb.structure.VortexEdgeProperty;
...
/**
 * StandardVortex is the entrance of the graph system, you can modify or
 * query the schema/vertex/edge data through this class.
 */

public class StandardVortex implements Vortex {

    public static final Class<?>[] PROTECT_CLASSES = {
            StandardVortex.class,
            StandardVortex.StandardVortexParams.class,
            TinkerPopTransaction.class,
            StandardVortex.Txs.class,
            StandardVortex.SysTransaction.class
    };
    ...
    private volatile VortexVariables variables;

    private final String name;

    private final StandardVortexParams params;

    private final VortexConfig configuration;
    ...
    private final VortexFeatures features;
    ...
    public StandardVortex(VortexConfig config) {
        this.params = new StandardVortexParams();
        ...
        this.features = new VortexFeatures(this, true);
        ...
        try {
            ...
            throw new VortexException(message);
        }
    ...
    private SchemaTransaction openSchemaTransaction() throws VortexException {
    ...
    private SysTransaction openSystemTransaction() throws VortexException {
        this.checkGraphNotClosed();
        try {
    ...
    private GraphTransaction openGraphTransaction() throws VortexException {
        // Open a new one
        this.checkGraphNotClosed();
        try {
    ...
    private AbstractSerializer serializer() {
        ...
        if (serializer == null) {
            throw new VortexException("Can't load serializer with name " + name);
        }
        return serializer;
    }
    ...
    public <I extends Io> I io(final Io.Builder<I> builder) {
        return (I) builder.graph(this).onMapper(mapper ->
                mapper.addRegistry(VortexIoRegistry.instance())
        ).create();
    }
    ...
    public void removeVertex(Vertex vertex) {
        this.graphTransaction().removeVertex((VortexVertex) vertex);
    }
    ...
    public void removeVertex(String label, Object id) {
        if (label != null) {
            ...
            if (!vl.existsIndexLabel()) {
                ...
                VortexVertex vertex = new VortexVertex(this, idValue, vl);
    ...
    public <V> void addVertexProperty(VertexProperty<V> p) {
        this.graphTransaction().addVertexProperty((VortexVertexProperty<V>) p);
    }

    @Override
    public <V> void removeVertexProperty(VertexProperty<V> p) {
        this.graphTransaction().removeVertexProperty((VortexVertexProperty<V>) p);
    }

    @Override
    public Edge addEdge(Edge edge) {
        return this.graphTransaction().addEdge((VortexEdge) edge);
    }
    ...
    public void removeEdge(Edge edge) {
        this.graphTransaction().removeEdge((VortexEdge) edge);
    }
    ...
    public void removeEdge(String label, Object id) {
        if (label != null) {
            ...
            if (!el.existsIndexLabel()) {
                ...
                Id idValue = VortexEdge.getIdValue(id, false);
                VortexEdge edge = new VortexEdge(this, idValue, el);
    ...
    public <V> void addEdgeProperty(Property<V> p) {
        this.graphTransaction().addEdgeProperty((VortexEdgeProperty<V>) p);
    }

    @Override
    public <V> void removeEdgeProperty(Property<V> p) {
        this.graphTransaction().removeEdgeProperty((VortexEdgeProperty<V>) p);
    }
    ...
    public void drop() {
        ...
        VortexConfig config = this.configuration();
    ...
    public VortexConfig cloneConfig(String newGraph) {
        VortexConfig config = (VortexConfig) this.configuration().clone();
    ...
    public synchronized Graph.Variables variables() {
        if (this.variables == null) {
            this.variables = new VortexVariables(this.params);
        }
    ...
    @Override
    public final void proxy(Vortex graph) {
    ...
    public void registerRpcServices(RpcServiceConfig4Server serverConfig,
        ...
        serverConfig.addService(this.name, clazz1, new VortexCacheNotifier(
        ...
        serverConfig.addService(this.name, clazz2, new VortexSchemaCacheNotifier(
    ...
    private void waitUntilAllTasksCompleted() {
        ...
        try {
            ...
            throw new VortexException("Failed to wait all tasks to complete", e);
        }
    }

    private class StandardVortexParams implements VortexParams {

        private Vortex graph = StandardVortex.this;

        private void graph(Vortex graph) {
            this.graph = graph;
        }
        ...
        @Override
        public String name() {
            return StandardVortex.this.name();
        }

        @Override
        public GraphMode mode() {
            return StandardVortex.this.mode();
        }

        @Override
        public GraphReadMode readMode() {
            return StandardVortex.this.readMode();
        }

        @Override
        public SchemaTransaction schemaTransaction() {
            return StandardVortex.this.schemaTransaction();
        }

        @Override
        public GraphTransaction systemTransaction() {
            return StandardVortex.this.systemTransaction();
        }

        @Override
        public GraphTransaction graphTransaction() {
            return StandardVortex.this.graphTransaction();
        }

        @Override
        public GraphTransaction openTransaction() {
            // Open a new one
            return StandardVortex.this.openGraphTransaction();
        }

        @Override
        public void closeTx() {
            StandardVortex.this.closeTx();
        }

        @Override
        public boolean started() {
            return StandardVortex.this.started();
        }

        @Override
        public boolean closed() {
            return StandardVortex.this.closed();
        }

        @Override
        public boolean initialized() {
            return StandardVortex.this.graphTransaction().storeInitialized();
        }

        @Override
        public BackendFeatures backendStoreFeatures() {
            return StandardVortex.this.backendStoreFeatures();
        }

        @Override
        public BackendStore loadSchemaStore() {
            return StandardVortex.this.loadSchemaStore();
        }

        @Override
        public BackendStore loadGraphStore() {
            return StandardVortex.this.loadGraphStore();
        }

        @Override
        public BackendStore loadSystemStore() {
            return StandardVortex.this.loadSystemStore();
        }

        @Override
        public EventHub schemaEventHub() {
            return StandardVortex.this.schemaEventHub;
        }

        @Override
        public EventHub graphEventHub() {
            return StandardVortex.this.graphEventHub;
        }

        @Override
        public EventHub indexEventHub() {
            return StandardVortex.this.indexEventHub;
        }

        @Override
        public VortexConfig configuration() {
            return StandardVortex.this.configuration();
        }

        @Override
        public ServerInfoManager serverManager() {
            // this.serverManager.initSchemaIfNeeded();
            return StandardVortex.this.serverInfoManager();
        }

        @Override
        public AbstractSerializer serializer() {
            return StandardVortex.this.serializer();
        }

        @Override
        public Analyzer analyzer() {
            return StandardVortex.this.analyzer();
        }

        @Override
        public RateLimiter writeRateLimiter() {
            return StandardVortex.this.writeRateLimiter;
        }

        @Override
        public RateLimiter readRateLimiter() {
            return StandardVortex.this.readRateLimiter;
        }

        @Override
        public RamTable ramtable() {
            return StandardVortex.this.ramtable;
        }
    }
    ...
    private class TinkerPopTransaction extends AbstractThreadLocalTransaction {
        ...
        private void verifyOpened() {
            if (!this.isOpen()) {
                throw new VortexException("Transaction has not been opened");
            }
        }
    ...
    private static class SysTransaction extends GraphTransaction {
        ...
        public SysTransaction(VortexParams graph, BackendStore store) {
            super(graph, store);
    ...
    private static class VortexSchemaCacheNotifier
            extends AbstractCacheNotifier
            implements SchemaCacheNotifier {

        public VortexSchemaCacheNotifier(EventHub hub, CacheNotifier proxy) {
            super(hub, proxy);
        }
    }

    private static class VortexCacheNotifier
            extends AbstractCacheNotifier
            implements GraphCacheNotifier {

        public VortexCacheNotifier(EventHub hub, CacheNotifier proxy) {
            super(hub, proxy);
        }
    }
