...
public class RolePermission {
    ...
    @JsonProperty("roles")
    private final Map<String, Map<VortexPermission, List<VortexResource>>> roles;

    public RolePermission() {
        this(new TreeMap<>());
    }

    private RolePermission(Map<String, Map<VortexPermission,
                                       List<VortexResource>>> roles) {
        this.roles = roles;
    }

    protected void add(String graph, String action,
                       List<VortexResource> resources) {
        this.add(graph, VortexPermission.valueOf(action), resources);
    }

    protected void add(String graph, VortexPermission action,
                       List<VortexResource> resources) {
        Map<VortexPermission, List<VortexResource>> permissions =
                                                this.roles.get(graph);
        if (permissions == null) {
            permissions = new TreeMap<>();
            this.roles.put(graph, permissions);
        }
        List<VortexResource> mergedResources = permissions.get(action);
    ...
    public Map<String, Map<VortexPermission, List<VortexResource>>> map() {
        return Collections.unmodifiableMap(this.roles);
    }

    public boolean contains(RolePermission other) {
        for (Map.Entry<String, Map<VortexPermission, List<VortexResource>>> e1 :
             other.roles.entrySet()) {
            String g = e1.getKey();
            Map<VortexPermission, List<VortexResource>> perms = this.roles.get(g);
            if (perms == null) {
                return false;
            }
            for (Map.Entry<VortexPermission, List<VortexResource>> e2 :
                e1.getValue().entrySet()) {
                List<VortexResource> ress = perms.get(e2.getKey());
                if (ress == null) {
                    return false;
                }
                for (VortexResource r : e2.getValue()) {
    ...
    public static RolePermission all(String graph) {
        return role(graph, VortexPermission.ANY);
    }

    public static RolePermission role(String graph, VortexPermission perm) {
        RolePermission role = new RolePermission();
        role.add(graph, perm, VortexResource.ALL_RES);
        return role;
    }
    ...
    private static class RolePermissionDeser
        ...
        public RolePermission deserialize(JsonParser parser,
            ...
            TypeReference<?> type = new TypeReference<TreeMap<String,
                             TreeMap<VortexPermission, List<VortexResource>>>>() {};
