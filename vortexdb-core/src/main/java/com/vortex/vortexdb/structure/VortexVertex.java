...
public class VortexVertex extends VortexElement implements Vertex, Cloneable {

    private static final List<VortexEdge> EMPTY_LIST = ImmutableList.of();

    private Id id;
    private VertexLabel label;
    protected Collection<VortexEdge> edges;

    public VortexVertex(final Vortex graph, Id id, VertexLabel label) {
        super(graph);
    ...
    protected List<Object> primaryValues() {
        ...
        for (Id pk : primaryKeys) {
            VortexProperty<?> property = this.getProperty(pk);
    ...
    public Collection<VortexEdge> getEdges() {
    ...
    public void addEdge(VortexEdge edge) {
    ...
    @Watched(prefix = "vertex")
    @Override
    public VortexEdge addEdge(String label, Vertex vertex, Object... keyValues) {
        VortexVertex targetVertex = (VortexVertex) vertex;

        VortexEdge edge = this.constructEdge(label, targetVertex, keyValues);
    ...
    public VortexEdge constructEdge(String label, VortexVertex vertex,
                                    Object... keyValues) {
        ElementKeys elemKeys = VortexElement.classifyKeys(keyValues);
        // Check id (must be null)
        if (elemKeys.id() != null) {
            throw Edge.Exceptions.userSuppliedIdsNotSupported();
        }
        Id id = VortexElement.getIdValue(elemKeys.id());

        // Check target vertex
        E.checkArgumentNotNull(vertex, "Target vertex can't be null");
        E.checkArgument(vertex instanceof VortexVertex,
                        "Target vertex must be an instance of VortexVertex");
        ...
        VortexEdge edge = new VortexEdge(this, id, edgeLabel, vertex);
     ...
     */
    @Watched
    public void addOutEdge(VortexEdge edge) {
     ...
     */
    @Watched
    public void addInEdge(VortexEdge edge) {
        if (edge.ownerVertex() == null) {
    ...
    public Iterator<Edge> getEdges(Directions direction, String... edgeLabels) {
        ...
        for (VortexEdge edge : this.edges) {
    ...
    public <V> VertexProperty<V> property(VertexProperty.Cardinality cardinality, String key, V value, Object... objects) {
        ...
        @SuppressWarnings("unchecked")
        VertexProperty<V> prop = (VertexProperty<V>) this.addProperty(propertyKey, value, !this.fresh());
        return prop;
    }
    ...
    @Watched(prefix = "vertex")
    @Override
    protected <V> VortexVertexProperty<V> newProperty(PropertyKey pkey, V val) {
        return new VortexVertexProperty<>(this, pkey, val);
    }
    ...
    protected <V> void onUpdateProperty(Cardinality cardinality,
        ...
        if (prop != null) {
            assert prop instanceof VortexVertexProperty;
            /*
            ...
            if (tx != null) {
                assert this.fresh();
                tx.addVertexProperty((VortexVertexProperty<V>) prop);
            } else {
                assert !this.fresh();
                this.graph().addVertexProperty((VortexVertexProperty<V>) prop);
            }
        }
    }
    ...
    public <V> Iterator<VertexProperty<V>> properties(String... keys) {
        // TODO: Compatible with TinkerPop properties() (Vortex-742)
     ...
     * @return a new vertex
     */
    public VortexVertex prepareRemoved() {
        // NOTE: clear edges/properties of the cloned vertex and return
        VortexVertex vertex = this.clone();
    ...
    @Override
    protected VortexVertex clone() {
        try {
            return (VortexVertex) super.clone();
        } catch (CloneNotSupportedException e) {
            throw new VortexException("Failed to clone VortexVertex", e);
        }
    }
    ...
    public static final Id getIdValue(Object idValue) {
        return VortexElement.getIdValue(idValue);
    }

    public static VortexVertex undefined(Vortex graph, Id id) {
        VertexLabel label = VertexLabel.undefined(graph);
        return new VortexVertex(graph, id, label);
    }

    public static VortexVertex create(final GraphTransaction tx,
                                      Id id, VertexLabel label) {
        return new VortexVertex4Insert(tx, id, label);
    }
    ...
    private static final class VortexVertex4Insert extends VortexVertex {

        private GraphTransaction tx;

        public VortexVertex4Insert(final GraphTransaction tx,
                                   Id id, VertexLabel label) {
