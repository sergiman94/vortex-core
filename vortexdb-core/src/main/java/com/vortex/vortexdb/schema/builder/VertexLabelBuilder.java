...
public class VertexLabelBuilder extends AbstractBuilder
    ...
    public VertexLabelBuilder(SchemaTransaction transaction,
                              Vortex graph, VertexLabel copy) {
        super(transaction, graph);
        E.checkNotNull(copy, "copy");
        Vortex origin = copy.graph();
        this.id = null;
        this.name = copy.name();
    ...
    public VertexLabel build() {
        Id id = this.validOrGenerateId(VortexType.VERTEX_LABEL,
                                       this.id, this.name);
    ...
    public VertexLabel create() {
        VortexType type = VortexType.VERTEX_LABEL;
    ...
    private static Set<String> mapPkId2Name(Vortex graph, Set<Id> ids) {
        return new HashSet<>(graph.mapPkId2Name(ids));
    }

    private static List<String> mapPkId2Name(Vortex graph, List<Id> ids) {
