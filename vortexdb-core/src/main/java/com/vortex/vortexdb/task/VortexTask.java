...
public class VortexTask<V> extends FutureTask<V> {
    ...
    public VortexTask(Id id, Id parent, String callable, String input) {
        this(id, parent, TaskCallable.fromClass(callable));
        this.input(input);
    }

    public VortexTask(Id id, Id parent, TaskCallable<V> callable) {
        super(callable);
    ...
    public String toString() {
        return String.format("VortexTask(%s)%s", this.id, this.asMap());
    }
    ...
    public boolean fail(Throwable e) {
        ...
        if (!(this.cancelled() && VortexException.isInterrupted(e))) {
    ...
    protected boolean checkDependenciesSuccess() {
        ...
        for (Id dependency : this.dependencies) {
            VortexTask<?> task = scheduler.task(dependency);
            if (!task.completed()) {
    ...
    public static <V> VortexTask<V> fromVertex(Vertex vertex) {
        ...
        VortexTask<V> task = new VortexTask<>((Id) vertex.id(), null, callable);
    ...
    private void checkPropertySize(int propertyLength, String propertyName) {
        ...
        Vortex graph = this.scheduler().graph();
    ...
    public void syncWait() {
        ...
        try {
            ...
            throw new VortexException("Failed to wait for task '%s' completed",
                                    e, this.id);
        }
        assert task != null;
        /*
        ...
        if (debugTest && !task.success()) {
            throw new VortexException("Task '%s' is failed with error: %s",
                                    task.id(), task.result());
        }
    }
