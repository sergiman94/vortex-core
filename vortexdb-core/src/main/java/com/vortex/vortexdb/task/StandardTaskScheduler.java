...
import com.vortex.vortexdb.task.VortexTask.P;
import com.vortex.vortexdb.task.TaskCallable.SysTaskCallable;
import com.vortex.vortexdb.task.TaskManager.ContextCallable;
...
public class StandardTaskScheduler implements TaskScheduler {
    ...
    private final Map<Id, VortexTask<?>> tasks;
    ...
    public StandardTaskScheduler(VortexParams graph,
    ...
    public <V> void restoreTasks() {
        ...
        for (TaskStatus status : TaskStatus.PENDING_STATUSES) {
            ...
            do {
                Iterator<VortexTask<V>> iter;
                for (iter = this.findTask(status, PAGE_SIZE, page);
                     iter.hasNext();) {
                    VortexTask<V> task = iter.next();
    ...
    private <V> Future<?> restore(VortexTask<V> task) {
    ...
    public <V> Future<?> schedule(VortexTask<V> task) {
        ...
        if (task.status() == TaskStatus.QUEUED) {
             ...
             * VortexTask.checkDependenciesSuccess() method
             */
    ...
    private <V> Future<?> submitTask(VortexTask<V> task) {
    ...
    private <V> Future<?> resubmitTask(VortexTask<V> task) {
    ...
    public <V> void initTaskCallable(VortexTask<V> task) {
        task.scheduler(this);
    ...
    @Override
    public synchronized <V> void cancel(VortexTask<V> task) {
        E.checkArgumentNotNull(task, "Task can't be null");
        ...
        throw new VortexException("Can't cancel task '%s' in status %s",
                                task.id(), task.status());
    }
    ...
    protected synchronized void scheduleTasks() {
        ...
        Collection<VortexServerInfo> scheduleInfos = this.serverManager()
                                                       .allServerInfos();
        String page = this.supportsPaging() ? PageInfo.PAGE_NONE : null;
        do {
            Iterator<VortexTask<Object>> tasks = this.tasks(TaskStatus.SCHEDULING,
                                                          PAGE_SIZE, page);
            while (tasks.hasNext()) {
                VortexTask<?> task = tasks.next();
                if (task.server() != null) {
                    // Skip if already scheduled
                    continue;
                }

                VortexServerInfo server = this.serverManager().pickWorkerNode(
                                        scheduleInfos, task);
                if (server == null) {
    ...
    protected void executeTasksOnWorker(Id server) {
        String page = this.supportsPaging() ? PageInfo.PAGE_NONE : null;
        do {
            Iterator<VortexTask<Object>> tasks = this.tasks(TaskStatus.SCHEDULED,
                                                          PAGE_SIZE, page);
            while (tasks.hasNext()) {
                VortexTask<?> task = tasks.next();
                this.initTaskCallable(task);
                ...
                VortexTask<?> memTask = this.tasks.get(task.id());
                if (memTask != null) {
    ...
    protected void cancelTasksOnWorker(Id server) {
        String page = this.supportsPaging() ? PageInfo.PAGE_NONE : null;
        do {
            Iterator<VortexTask<Object>> tasks = this.tasks(TaskStatus.CANCELLING,
                                                          PAGE_SIZE, page);
            while (tasks.hasNext()) {
                VortexTask<?> task = tasks.next();
                Id taskServer = task.server();
    ...
    protected void taskDone(VortexTask<?> task) {
        this.remove(task);
    ...
    protected void remove(VortexTask<?> task) {
        E.checkNotNull(task, "remove task");
        VortexTask<?> delTask = this.tasks.remove(task.id());
    ...
    public <V> void save(VortexTask<V> task) {
        ...
        this.call(() -> {
            ...
            VortexVertex vertex = this.tx().constructVertex(task);
    ...
    public <V> VortexTask<V> task(Id id) {
        ...
        VortexTask<V> task = (VortexTask<V>) this.tasks.get(id);
        if (task != null) {
            return task;
        }
    ...
    public <V> Iterator<VortexTask<V>> tasks(List<Id> ids) {
        ...
        List<VortexTask<V>> taskInMem = new ArrayList<>();
        for (Id id : ids) {
            @SuppressWarnings("unchecked")
            VortexTask<V> task = (VortexTask<V>) this.tasks.get(id);
            if (task != null) {
        ...
        ExtendableIterator<VortexTask<V>> iterator;
    ...
    public <V> VortexTask<V> findTask(Id id) {
        VortexTask<V> result =  this.call(() -> {
            Iterator<Vertex> vertices = this.tx().queryVertices(id);
            Vertex vertex = QueryResults.one(vertices);
            if (vertex == null) {
                return null;
            }
            return VortexTask.fromVertex(vertex);
        });
        if (result == null) {
    ...
    public <V> Iterator<VortexTask<V>> findTasks(List<Id> ids) {
        return this.queryTask(ids);
    }

    public <V> Iterator<VortexTask<V>> findAllTask(long limit, String page) {
        return this.queryTask(ImmutableMap.of(), limit, page);
    }

    public <V> Iterator<VortexTask<V>> findTask(TaskStatus status, long limit, String page) {
    ...
    public <V> VortexTask<V> delete(Id id) {
        ...
        VortexTask<?> task = this.task(id);
        /*
         * The following is out of date when task running on worker node:
         * VortexTask<?> task = this.tasks.get(id);
        ...
        return this.call(() -> {
            Iterator<Vertex> vertices = this.tx().queryVertices(id);
            VortexVertex vertex = (VortexVertex) QueryResults.one(vertices);
            if (vertex == null) {
                return null;
            }
            VortexTask<V> result = VortexTask.fromVertex(vertex);
    ...
    private <V> VortexTask<V> waitUntilTaskCompleted(Id id, long seconds,
                                                     long intervalMs)
                                                   throws TimeoutException {
        long passes = seconds * 1000 / intervalMs;
        VortexTask<V> task = null;
    ...
    private <V> Iterator<VortexTask<V>> queryTask(String key, Object value,
                                                  long limit, String page) {
        return this.queryTask(ImmutableMap.of(key, value), limit, page);
    }

    private <V> Iterator<VortexTask<V>> queryTask(Map<String, Object> conditions,
                                                  long limit, String page) {
        return this.call(() -> {
            ...
            Iterator<VortexTask<V>> tasks =
                    new MapperIterator<>(vertices, VortexTask::fromVertex);
            // Convert iterator to list to avoid across thread tx accessed
            return QueryResults.toList(tasks);
        });
    }

    private <V> Iterator<VortexTask<V>> queryTask(List<Id> ids) {
        return this.call(() -> {
    ...
    private <V> V call(Callable<V> callable) {
        ...
        try {
            ...
            throw new VortexException("Failed to update/query TaskStore: %s",
                                    e, e.toString());
        }
    }
    ...
    private void checkOnMasterNode(String op) {
        if (!this.serverManager().master()) {
            throw new VortexException("Can't %s task on non-master server", op);
        }
    }
    ...
    private static class TaskTransaction extends GraphTransaction {
        ...
        public TaskTransaction(VortexParams graph, BackendStore store) {
            super(graph, store);
            this.autoCommit(true);
        }

        public VortexVertex constructVertex(VortexTask<?> task) {
            if (!this.graph().existsVertexLabel(TASK)) {
                throw new VortexException("Schema is missing for task(%s) '%s'",
                                        task.id(), task.name());
            }
            return this.constructVertex(false, task.asArray());
        }

        public void deleteIndex(VortexVertex vertex) {
            // Delete the old record if exist
            Iterator<Vertex> old = this.queryVertices(vertex.id());
            VortexVertex oldV = (VortexVertex) QueryResults.one(old);
            if (oldV == null) {
                return;
            }
        ...
        private boolean deleteIndexIfNeeded(VortexVertex oldV, VortexVertex newV) {
        ...
        private IndexLabel createIndexLabel(VertexLabel label, String field) {
            ...
            IndexLabel indexLabel = schema.indexLabel(name)
                                          .on(VortexType.VERTEX_LABEL, TASK)
                                          .by(field)
                                          .build();
