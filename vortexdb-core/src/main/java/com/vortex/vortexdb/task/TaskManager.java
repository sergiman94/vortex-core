...
public final class TaskManager {
    ...
    private final Map<VortexParams, TaskScheduler> schedulers;
    ...
    private TaskManager(int pool) {
        ...
        this.schedulerExecutor = ExecutorUtil.newPausableScheduledThreadPool(
                                 1, TASK_SCHEDULER);
        // Start after 10s waiting for VortexServer startup
        this.schedulerExecutor.scheduleWithFixedDelay(this::scheduleOrExecuteJob,
                                                      10L, SCHEDULE_PERIOD,
                                                      TimeUnit.SECONDS);
    }

    public void addScheduler(VortexParams graph) {
    ...
    public void closeScheduler(VortexParams graph) {
    ...
    private void closeTaskTx(VortexParams graph) {
        ...
        try {
            ...
            throw new VortexException("Exception when closing task tx", e);
        }
    }

    private void closeSchedulerTx(VortexParams graph) {
        ...
        try {
            ...
            throw new VortexException("Exception when closing scheduler tx", e);
        }
    }
    ...
    public TaskScheduler getScheduler(VortexParams graph) {
        return this.schedulers.get(graph);
    }

    public ServerInfoManager getServerInfoManager(VortexParams graph) {
    ...
    public void shutdown(long timeout) {
        ...
        if (ex != null) {
            throw new VortexException("Failed to wait for TaskScheduler", ex);
        }
    }
    ...
    protected void notifyNewTask(VortexTask<?> task) {
