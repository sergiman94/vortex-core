...
public interface VortexAuthenticator extends Authenticator {
    ...
    public default User authenticate(final Map<String, String> credentials)
        ...
        VortexAuthProxy.logUser(user, credentials.get(KEY_PATH));
        /*
         * Set authentication context
         * TODO: unset context after finishing a request
         */
        VortexAuthProxy.setContext(new Context(user));
    ...
    public static VortexAuthenticator loadAuthenticator(VortexConfig conf) {
        String authClass = conf.get(ServerOptions.AUTHENTICATOR);
        if (authClass.isEmpty()) {
            return null;
        }

        VortexAuthenticator authenticator;
        ClassLoader cl = conf.getClass().getClassLoader();
        try {
            authenticator = (VortexAuthenticator) cl.loadClass(authClass)
                                                  .newInstance();
        } catch (Exception e) {
            throw new VortexException("Failed to load authenticator: '%s'",
                                    authClass, e);
        }
    ...
    public static class RolePerm {
        ...
        private Map<String, Map<VortexPermission, Object>> roles;

        public RolePerm() {
            this.roles = new HashMap<>();
        }

        public RolePerm(Map<String, Map<VortexPermission, Object>> roles) {
            this.roles = roles;
        }
        ...
        private boolean matchResource(VortexPermission requiredAction,
             ...
             */
            if (VortexResource.allowed(requiredResource)) {
                return true;
            }

            String owner = requiredResource.graph();
            Map<VortexPermission, Object> permissions = this.roles.get(owner);
            if (permissions == null) {
                return false;
            }
            ...
            List<VortexResource> ress;
            if (permission instanceof List) {
                @SuppressWarnings("unchecked")
                List<VortexResource> list = (List<VortexResource>) permission;
                ress = list;
            } else {
                ress = VortexResource.parseResources(permission.toString());
            }
            for (VortexResource res : ress) {
        ...
        private static Object matchedAction(VortexPermission action,
                                            Map<VortexPermission, Object> perms) {
            Object matched = perms.get(action);
            if (matched != null) {
                return matched;
            }
            for (Map.Entry<VortexPermission, Object> e : perms.entrySet()) {
                VortexPermission permission = e.getKey();
                // May be required = ANY
        ...
        public static boolean match(Object role, RequiredPerm requiredPerm) {
            ...
            if (requiredPerm.action() == VortexPermission.NONE) {
        ...
        public static boolean match(Object role, VortexPermission required,
        ...
        public static boolean match(Object role, RolePermission grant,
            ...
            if (resourceObject != null) {
                ...
                if (element instanceof VortexUser &&
                    ((VortexUser) element).name().equals(USER_ADMIN)) {
    ...
    public static class RequiredPerm {
        ...
        @JsonProperty("action")
        private VortexPermission action;
        @JsonProperty("resource")
        private ResourceType resource;

        public RequiredPerm() {
            this.owner = "";
            this.action = VortexPermission.NONE;
        ...
        public VortexPermission action() {
            return this.action;
        }
        ...
        private void parseAction(String action) {
            ...
            this.action = VortexPermission.valueOf(action.toUpperCase());
        }

        public static String roleFor(String owner, VortexPermission perm) {
            /*
