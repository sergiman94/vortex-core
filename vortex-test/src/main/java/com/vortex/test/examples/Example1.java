...
public class Example1 {
    ...
    public static void main(String[] args) throws Exception {
        ...
        Vortex graph = ExampleUtil.loadGraph();
    ...
    private static void thread(Vortex graph) throws InterruptedException {
        Thread t = new Thread(() -> {
            // Default tx
    ...
    public static void showFeatures(final Vortex graph) {
        LOG.info("SupportsPersistence: {}",
                 graph.features().graph().supportsPersistence());
    }

    public static void loadSchema(final Vortex graph) {
    ...
    public static void loadData(final Vortex graph) {
    ...
    public static void testQuery(final Vortex graph) {
        // query all
        ...
        ConditionQuery q = new ConditionQuery(VortexType.VERTEX);
        PropertyKey age = graph.propertyKey("age");
        q.key(VortexKeys.PROPERTIES, age.id());
        ...
        q = new ConditionQuery(VortexType.EDGE);
        q.eq(VortexKeys.OWNER_VERTEX, IdGenerator.of(authorId));
        q.eq(VortexKeys.DIRECTION, Directions.OUT);
        q.eq(VortexKeys.LABEL, authored.id());
        q.eq(VortexKeys.SORT_VALUES, "");
        q.eq(VortexKeys.OTHER_VERTEX, IdGenerator.of(book1Id));
        ...
        if (graph.backendStoreFeatures().supportsQueryWithContainsKey()) {
            ...
            q.key(VortexKeys.PROPERTIES, contribution.id());
    ...
    public static void testRemove(final Vortex graph) {
    ...
    public static void testVariables(final Vortex graph) {
        // variables test
    ...
    public static void testLeftIndexProcess(final Vortex graph) {
