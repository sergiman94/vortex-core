...
public class BaseApiTest {
    ...
    private static final Logger LOG = Log.logger(BaseApiTest.class);

    private static String BASE_URL = "http://127.0.0.1:8080";
    private static String GRAPH = "vortex";
    ...
    public static void init() {
        LOG.info("Initializing the base api class ... ");
        client = newClient();
    ...
    protected static String getVertexId(String label, String key, String value)
        ...
        if (list.size() != 1) {
            throw new VortexException("Failed to get vertex id: %s", content);
        }
    ...
    protected static void waitTaskSuccess(int task) {
        String status;
        do {
            Response r = client.get("/graphs/vortex/tasks/",
                                    String.valueOf(task));
            String content = assertResponseStatus(200, r);
    ...
    protected static <T> List<T> readList(String content,
        ...
        try {
            ...
            if (element == null) {
                throw new VortexException(String.format(
